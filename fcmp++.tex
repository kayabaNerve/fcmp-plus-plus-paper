...
$\dloglhs(n_{y_{0..2}}, n_{yx_{0..m}}, n_{x_{1..n}}, n_0, d_{y_{0..2}}, d_{yx_{0..m}}, d_{x_{0..n}}, d_0, \delta, \chl) \rightarrow$

$~~p_{0_n} = \sum^2_{i=0} \chl\mathsf{.y}^{(i+1)} * n_{y_{i}} + \sum^m_{i=0} (\chl\mathsf{.y} * \chl\mathsf{.x}^{(i+1)}) * n_{xy_{i}} + \sum^n_{i=1} \chl\mathsf{.x}^{(i+1)} * n_{x_{i}} + 1 * \chl\mathsf{.x} + 1 * n_0$

$~~p_{0_d} = \sum^2_{i=0} \chl\mathsf{.y}^{(i+1)} * d_{y_{i}} + \sum^m_{i=0} (\chl\mathsf{.y} * \chl\mathsf{.x}^{(i+1)}) * d_{xy_{i}} + \sum^n_{i=0} \chl\mathsf{.x}^{(i+1)} * d_{x_{i}} + 1 * d_0$
...
$n, d$ are the coefficients for the numerator and the denominator of the first fraction from formula 1 of https://eprint.iacr.org/2022/596. $d$ is checked to be non-zero by the creation of the inverse, and $n$ is enforced to be non-zero by using $1$ for the coefficient for $x^1$.
...
$\dlog(n_{y_{0..2}}, n_{yx_{0..m}}, n_{x_{1..n}}, n_0, d_{y_{0..2}}, d_{yx_{0..m}}, d_{x_{0..n}}, d_0, s_{0..255}, x, y) \rightarrow$
...
$~~\sum^2_{i=0} \dloglhs(n_y, n_{yx}, n_x, n_0, d_y, d_{yx}, d_x, d_0, \delta, \chl_i) ~+~ \sum^{254}_{i=0} (\mu ~-~ (G_i\mathsf{.y} ~-~ (\delta ~*~ G_i\mathsf{.x})))^{-1} ~*~ s_i = 0$
...
$\dlogpok(n_{y_{0..2}}, n_{yx_{0..m}}, n_{x_{1..n}}, n_0, d_{y_{0..2}}, d_{yx_{0..m}}, d_{x_{0..n}}, d_0, s_{0..255}, x, y) \rightarrow$

$~~\dlog(n_y, n_{yx}, n_x, n_0, d_y, d_{yx}, d_x, d_0, s, x, y)$
