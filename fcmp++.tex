...
We replace this with a composition proving the following statement.

$\{$

$~~G, H, T \in \mathbb{G}, \tilde{C}, L \in \mathbb{G}, S \in (\mathbb{G}, \mathbb{G}, \mathbb{G})^n;$

$~~O, I, C \in \mathbb{G}, x, y, r_c \in \mathbb{F} ~|$

$~~(O, I, C) \in S,~$

$~~O = x \cdot G + y \cdot T,~ L = x \cdot I,$

$~~\tilde{C} = C + (r_c \cdot G)$

$\}$

A $y$ term is introduced, which is $0$ for all prior witnesses. $C$ is no longer opened, solely re-randomized ($C$ uses $G$ for blinding and $H$ for binding, contrary to popular convention). This is accepted as the $C$ terms present in the set are already known to be well-formed (over $G$ and $H$) and the prover does prove with the Balance Proof their $C$ terms' values' sum is the value spent by the transaction.
...
This design was proven secure, and optimized, in https://github.com/cypherstack/fcmp-review.

\section{Membership Proof}
...
Bulletproofs are a trustless proof already used within Monero, which can be used to prove arithmetic circuit proofs are satisfied. We solely need a hash function for the Merkle tree, the most efficient for our use-case being a Pedersen hash.
...
Creating Pedersen hashes in-circuit would be suboptimal. Even with native arithmetic, by having the Bulletproof be proven over a curve whose scalar field is the field of the curve the hash is performed over, we'd have a cost of \~10 in-circuit multiplications per word hashed.

We instead use Generalized Bulletproofs, a modification to Bulletproofs recently proven secure: https://github.com/cypherstack/generalized-bulletproofs

Generalized Bulletproofs can be used to output Pedersen hashes at effectively no additional cost, although it cannot operate over its hashes (and we do need to operate over a series of hashes, due to utilizing a Merkle tree). We solve this by operating over output hashes with a new Generalized Bulletproof, this one on a curve whose scalar field is the prior curve's field. This continues for each layer of the Merkle tree, and is most efficiently realizable with a curve cycle (a curve whose scalar field is the field of a curve whose scalar field is the field for the original curve). With a curve cycle, one can alternate between the two curves (and two proofs) without needing additional curves/proofs per layer.

The ability to produce Pedersen hashes also enables forming challenges off partial transcripts of the witness. By defining a Pedersen hash of a set of variables within the circuit, we can hash that outside of the circuit with a traditional hash function to obtain a challenge usable for the rest of the circuit. This allows interactive proofs within the circuit (due to the Fiat-Shamir transform), and with it, more efficient gadgets.
...
$~~~~\forall ~0 <= j < L_i\mathsf{.length}():$

$~~~~~~L_i = \sum^{L_i\mathsf{.length}()}_{j=0} chl_j * L_{i,j}$
...
$~~p_{0_d} = \chl\mathsf{.y} * d_y + \sum^m_{i=0} (\chl\mathsf{.y} * \chl\mathsf{.x}^{(i+1)}) * d_{yx_{i}} + \sum^n_{i=1} \chl\mathsf{.x}^{(i+1)} * d_{x_{i}} + 1 * \chl\mathsf{.x} + 1 * d_0$
...
$~~\tuplememberlist(L, (o_x, o_y, i_x, i_y, c_x, c_y))$
...
Please note $H$ is the blinding generator for vector commitments on the previous curve (not the unblinded hash, which is $h_x, h_y$).

\subsection{Tying It All Together}

The first layer (performed within a Generalized Bulletproof over the Selene curve) proves knowledge of the existence of $O, I, C$ points before being re-randomized by solely the intended blinding generators for each element, and that $R$ is well-formed. It then proves the points are members of the list hashed into a vector commitment. It does not prove nothing else is hashed into the vector commitment (it doesn't constrain other members of the commitment are equal to zero) as that is unnecessary.

The prover and verifier take the vector commitment, a point on the Selene curve, and add $B_{\mathsf{selene}}$ (a public parameter sampled uniformly from random). The resulting point is passed as $\tilde{H}$ to the $\mathsf{AdditionalLayer}$ gadget within the Generalized Bulletproof over the Helios curve if another layer exists.

$\mathsf{AdditionalLayer}$ proves knowledge of a point $h_x, h_y$ before re-randomizing by solely the intended blinding generator $H$. It then proves the $x$-coordinate is a member of the list hashed into a vector commitment. The prover and verifier take this vector commitment, add $B_{\mathsf{curve}}$, where $\mathsf{curve}$ is the curve the vector commitment is on, and passes the result to the $\mathsf{AdditionalLayer}$ gadget within the Generalized Bulletproof over the reciprocal curve if another layer exists. If this vector commitment is for the root of the Merkle tree, the prover performs a discrete-log proof of knowledge for the difference of their vector commitment and the declared Merkle tree's root over the blinding generator for the vector commitment.

The verifier additionally checks the declared Merkle tree's root is acceptable (on-chain) and that the amount of layers executed is the amount of layers expected for the Merkle tree.

\subsection{On Only Hashing $x$-coordinates Within $\mathsf{AdditionalLayer}$}

At each additional layer, only $x$-coordinates of the vector commitments are hashed for efficiency's sake. This would enable proving that either a branch $\tilde{H}$ is in-tree or its additive inverse, $-\tilde{H}$. We prevent that not by hashing $\tilde{H}$ yet $B_{\mathsf{curve}} + \tilde{H}$. Attempting to prove for $-\tilde{H}$ attempts to prove $B_{\mathsf{curve}} - \tilde{H}$ is in-tree, where $B_{\mathsf{curve}} + \tilde{H}$ and $B_{\mathsf{curve}} - \tilde{H}$ have distinct $x$-coordinates. While there would be an $x$-coordinate collision if $\tilde{H}$ had $2 \cdot B_{\mathsf{curve}}$ subtracted, the Generalized Bulletproof proves knowledge of an opening of $\tilde{H}$ over a specific set of generators (preventing inclusion of $B_{\mathsf{curve}}$ terms).

Credit for the idea of $B_{\mathsf{curve}}$ goes to tevador.
...
Then, for each $w_a$ chunk of $\mathsf{outputs_1}$, padding the last chunk with $Z$ if incomplete, hash the flattened chunk, add $B_{\mathsf{curve}}$, and append the resulting point to the next layer.

Please note this technically makes the width of the first layer $6 * w_a$, due to considering six field elements (each one word of the hash function) as individual leaves.
...
This byte buffer contains one Generalized Schnorr Protocol per input (as they don't sufficiently benefit from being merged into a single proof).

It also contains one Generalized Bulletproof, containing the circuit evaluations for each input. This Generalized Bulletproof must be padded to the nearest power of two, which does computation to be wasted (one Generalized Bulletproof for 9 inputs has the verification cost of 16 inputs). The solution to this is to introduce a fee clawback, as we have for the Bulletproof(+) used to prove the outputs' amount commitments are within range, and an input limit, as we have an output limit.
...
As of the most recent hard fork (Bulletproofs+), the only fields from prunable which are hashed when signing are the Bulletproofs. Moving forward, no fields from prunable are hashed for the purposes of signing. RingCT base is modified to exclude $\mathsf{ReferenceBlock}$ when being serialized for the hash for signing. While this would imply $\mathsf{ReferenceBlock}$ should be placed within prunable, this field cannot be pruned (as it's necessary to evaluate if the transaction should be dropped upon deep reorganization). The lack of hashing it, and malleability regarding the tree used, is explicitly allowed in order to support transaction chaining.
...
For a write-up on doing this with CLSAG and the existing monero-wallet crate, please refer to https://gist.github.com/kayabaNerve/3b2c648c623bc4ce4ca288725428ea76.
