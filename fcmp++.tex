...
In-circuit, we consider a $y$ coordinate permissible if it has a square root. This can be checked in-circuit with just one multiplication, making it incredibly efficient. Out-of-circuit, we consider a $y$ coordinate permissible if $\mathsf{permissible}(y)$ and $!\mathsf{permissible}(-y)$.
...
$\dlogpok_G(s_{0..256}, x, y) \rightarrow$
...
$\dlog$ takes the logarithmic derivative of $D(C_0) * D(C_1) * D(C_2) = \prod_{i=0}^{255}(\mu - Z(G_i))$, where $D$ is the divisor, to enable evaluation with a linear combination (instead of hundreds of in-circuit multiplications).

Eagen's work has an $x$-only divisor equation of $D(C) * D(-C) = \prod_{i=0}^{255}(C.x - G_i.x)$. If we instead take the logarithmic derivative of that, the proof loses the ability to differentiate $G_i.x$ from $(-G_i).x$. We can use this to generate a trinary system $-1, 0, 1$ with $162$ powers of $G$ ($G, 3 \cdot G, 9 \cdot G$) which generates nearly the same $2^{256}$ group, yet with only $162$ trits (which can be collapsed to bits due to the lack of ability to distinguish $-1$ from $1$). Not only does mean less work calculating the divisor, the evaluation of the challenge is also simpler.
...
For an input tuple $(O, I, C)$, $O, C$ must be permissible (as defined by the permissible gadget) before they can be hashed into the tree. $O$ has $T$ added until it's permissible, where for $O = x \cdot G + y \cdot T$, $y$ is incremented as many times as $T$ was added. $C$ has $G$ added until it's permissible, where for $C = r \cdot G + a \cdot H$, $r$ (the commitment mask) is incremented as many times as $G$ was added.
...
We could also require output keys be permissible, yet doing so would interfere with the current scanning protocol
