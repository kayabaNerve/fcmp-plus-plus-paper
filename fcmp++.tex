...
\subsubsection{Discrete Logarithm Bitstring}

\newcommand{\dlogbits}{\mathsf{DiscreteLogarithmBitstring}}

$\dlogbits(d_{0..255}) \rightarrow$

$~~\forall ~0 <= i < 255:$

$~~~~\bit(d_i)$

$~~\inequality(\sum^{254}_{i=0} d_i, 0)$

$~~\inequality(\sum^{254}_{i=0} d_i, 1)$

This proves a claimed discrete logarithm (represented as a bitstring) is in fact a bitstring. It also proves that at least two bits are set, enabling unchecked incomplete addition with any power-of-two generator.
...
This has the precondition $s$ is constrained to be a valid bitstring with more than one bit set.

For a generator $G$, both the prover and the verifier calculate $G_0 = 2^0 \cdot G, ...,G_{254} = 2^{254} \cdot G$. The prover provides $s$, the 255-bit decomposition of a scalar for the embedded elliptic curve, and the divisor interpolating $s \cdot G$ with the relevant powers of $G$ within Pedersen vector commitments.
